...
def _is_valid_github_login(value: str) -> bool:
    """True bila value adalah username GitHub yang valid ATAU email yang valid.

    GitHub username: 1-39 karakter, huruf/angka/tanda-hubung, tidak diawali/diakhiri
    tanda hubung, tidak ada tanda hubung berturut-turut.
    Untuk keperluan validasi sederhana kita hanya cek:
      - minimal 1 karakter
      - tidak mengandung spasi
      - bila ada '@' → harus valid sebagai email
      - bila tidak ada '@' → dianggap username GitHub (format bebas)
    """
    v = (value or "").strip()
    if not v or " " in v:
        return False
    ...
    # Username GitHub: boleh huruf, angka, tanda hubung; minimal 1 karakter
    return bool(re.match(r"^[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?$", v))
...
async def entry_do_claim(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if remaining > 1:
        prompt = (
            ...
            f"`username:password`\n"
            f"`email:password`\n"
            f"`username:password:TOTP_SECRET`\n"
...
async def handle_do_ghs(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not ghs_accounts:
        await update.effective_chat.send_message(
            ...
            "Gunakan: `username:password` atau `email:password` "
            "(satu akun per baris).\n"
    ...
    # Validasi username/email pada tiap akun GHS.
    invalid = [login for login, _, _ in ghs_accounts if not _is_valid_github_login(login)]
    if invalid:
        await update.effective_chat.send_message(
            "❌ Ada login GHS yang tidak valid (username atau email):\n"
