...
async def _notify_admins(
    bot, text_md: str, text_plain: str | None = None, exclude_user_id: int | None = None
) -> None:
    ...
    exclude_user_id: bila buyer juga seorang admin, jangan kirim notif admin
    ke dirinya (agar tidak dapat pesan ganda — buyer cukup terima pesan produk).
    """
...
def kb_admin_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "💰 Kelola Saldo", callback_data="admin_balance"
                )
            ],
            [
                InlineKeyboardButton(
...
# ------------------------------------------------------------------
# Kelola Saldo — ConversationHandler (tambah / set saldo user)
# ------------------------------------------------------------------


async def entry_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Entry: admin klik 'Kelola Saldo' → minta perintah saldo."""
    query = update.callback_query
    assert query is not None
    assert update.effective_user is not None
    if not is_admin(update.effective_user.id):
    ...
    await query.edit_message_text(
        "💰 *Kelola Saldo User*\n\n"
        "Kirim perintah dalam *satu baris*:\n\n"
        "`add | <user_id atau @username> | <jumlah>`\n"
        "`set | <user_id atau @username> | <jumlah>`\n\n"
        "Atau tanpa pipe:\n"
        "`add @username 50000`\n"
        "`set @username 0`\n\n"
        "Contoh:\n"
        "`add | @budi | 50000` — tambah saldo Rp50.000\n"
        "`add @budi 50000` — sama, tanpa pipe\n"
        "`add | 123456789 | -10000` — kurangi saldo Rp10.000\n"
        "`set | @budi | 0` — set saldo jadi Rp0\n\n"
        "Atur harga *Jasa Klaim Kupon DO*:\n"
        "`price | <jumlah>` — mis. `price | 5000`\n\n"
        "_Username harus pernah memakai bot agar terdeteksi._\n\n"
        "Kirim /cancel untuk membatalkan.",
        parse_mode="Markdown",
    )
...
async def handle_balance_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Terima perintah saldo (add/set) dan terapkan ke user."""
    assert update.effective_user is not None
    assert update.message is not None
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    ...
    raw = (update.message.text or "").strip()
    # Support both pipe-separated ("add | @user | 50000")
    # and space-separated ("add @user 50000") formats.
    if "|" in raw:
        parts = [p.strip() for p in raw.split("|")]
    else:
        parts = raw.split(None, 2)  # split on whitespace, max 3 parts

    # Atur harga jasa klaim kupon: "price | <jumlah>" or "price <jumlah>"
    if len(parts) == 2 and parts[0].lower() == "price":
        price_clean = parts[1].replace(".", "").replace(",", "")
        if not price_clean.isdigit():
            await update.message.reply_text(
                "⚠️ Harga harus angka (contoh: `5000`). Coba lagi, atau /cancel.",
                parse_mode="Markdown",
            )
            return WAITING_BALANCE_INPUT
        db.set_setting("coupon_service_price", int(price_clean))
        await update.message.reply_text(
            f"✅ Harga *Jasa Klaim Kupon DO* di-set ke *{fmt_price(int(price_clean))}*.",
            parse_mode="Markdown",
            reply_markup=kb_admin_back(),
        )
        return ConversationHandler.END
    ...
    if len(parts) != 3:
        await update.message.reply_text(
            ...
            "`add | <user_id/@username> | <jumlah>`\n"
            "`set | <user_id/@username> | <jumlah>`\n"
            "`price | <jumlah>`\n\n"
            "Atau tanpa pipe:\n"
            "`add @username 50000`\n"
            "`set @username 0`\n\n"
    ...
    action, target, amount_raw = parts[0].lower(), parts[1], parts[2]
    if action not in ("add", "set"):
        await update.message.reply_text(
            "⚠️ Aksi harus `add` atau `set`. Coba lagi, atau /cancel.",
            parse_mode="Markdown",
        )
        return WAITING_BALANCE_INPUT

    # Parse jumlah (boleh negatif untuk add)
    amount_clean = amount_raw.replace(".", "").replace(",", "")
    try:
        amount = int(amount_clean)
    except ValueError:
        await update.message.reply_text(
            "⚠️ Jumlah harus angka (contoh: `50000`). Coba lagi, atau /cancel.",
            parse_mode="Markdown",
        )
        return WAITING_BALANCE_INPUT

    # Resolusi target: user_id numerik, atau @username
    target_uid = None
    target_uname = ""
    if target.lstrip("-").isdigit():
        target_uid = int(target)
    else:
        u = db.find_user_by_username(target)
        if not u:
            await update.message.reply_text(
                f"⚠️ User `{_escape_md(target)}` tidak ditemukan.\n"
                "User harus pernah memakai bot. Coba lagi, atau /cancel.",
                parse_mode="Markdown",
            )
            return WAITING_BALANCE_INPUT
        target_uid = u["user_id"]
        target_uname = u.get("username", "")

    if action == "add":
        new_balance = db.add_balance(target_uid, amount, target_uname)
        verb = "ditambah" if amount >= 0 else "dikurangi"
        change_line = f"Saldo {verb}: *{fmt_price(abs(amount))}*"
    else:  # set
        new_balance = db.set_balance(target_uid, amount, target_uname)
        change_line = f"Saldo di-set ke: *{fmt_price(max(0, amount))}*"

    await update.message.reply_text(
        f"✅ *Saldo Diperbarui*\n\n"
        f"👤 User: `{target_uid}`"
        + (f" (@{_escape_md(target_uname)})" if target_uname else "")
        + f"\n{change_line}\n"
        f"💰 Saldo sekarang: *{fmt_price(new_balance)}*",
        parse_mode="Markdown",
        reply_markup=kb_admin_back(),
    )

    # Beri tahu user kalau memungkinkan
    try:
        await context.bot.send_message(
            chat_id=target_uid,
            text=(
                f"💰 *Saldo kamu diperbarui oleh admin.*\n\n"
                f"Saldo sekarang: *{fmt_price(new_balance)}*"
            ),
            parse_mode="Markdown",
        )
    ...
    logger.info(
        "Saldo %s oleh admin %s: user=%s amount=%s -> %s",
        action, update.effective_user.id, target_uid, amount, new_balance,
    )
    return ConversationHandler.END
...
async def auto_confirm_order(order_id: str, bot) -> bool:
    ...
    # ── Hapus pesan QRIS setelah pembayaran lunas ───────────────────
    qris_msg_id = order.get("pakasir_qris_msg_id")
    qris_chat_id = order.get("pakasir_qris_chat_id")
    if qris_msg_id and qris_chat_id:
        try:
            await bot.delete_message(chat_id=qris_chat_id, message_id=qris_msg_id)
        except Exception as exc:
            logger.debug("auto_confirm: gagal hapus pesan QRIS order %s: %s", order_id, exc)

    # ── TOP-UP SALDO ────────────────────────────────────────────────
    # Order top-up tidak mengirim akun; cukup kreditkan saldo ke user.
    if order.get("is_topup"):
        amount = int(order.get("topup_amount") or order.get("price") or 0)
        new_balance = db.add_balance(order["user_id"], amount, order.get("username", ""))
        db.update_order(order_id, status="confirmed")
        try:
            await bot.send_message(
                chat_id=order["user_id"],
                text=(
                    f"✅ *Top Up Saldo Berhasil!*\n\n"
                    f"💰 Saldo masuk: *{fmt_price(amount)}*\n"
                    f"💳 Saldo sekarang: *{fmt_price(new_balance)}*\n\n"
                    f"Order ID: `{order_id}`"
                ),
                parse_mode="Markdown",
            ...
            logger.error("auto_confirm topup: gagal notif user %s: %s", order["user_id"], exc)
        admin_md = (
            f"💰 *Top Up Terkonfirmasi*\n\n"
            f"Order: `{order_id}`\n"
            f"User: @{_escape_md(order['username'])}\n"
            f"Jumlah: {fmt_price(amount)}"
        )
        await _notify_admins(bot, admin_md, exclude_user_id=order["user_id"])
        logger.info("auto_confirm: topup %s confirmed (+%s)", order_id, amount)
        return True

    # ── JASA KLAIM KUPON DO (dibayar via QRIS) ──────────────────────
    if order.get("is_coupon_service"):
        db.update_order(order_id, status="confirmed")
        try:
            # Pembayaran dulu — kredensial DO diinput setelah lunas via tombol.
            from handlers.coupon_claim import send_coupon_resume_prompt

            await send_coupon_resume_prompt(order, bot)
        except Exception as exc:
            logger.exception("auto_confirm coupon: gagal proses %s: %s", order_id, exc)
        admin_md = (
            f"🎟️ *Jasa Klaim Kupon Dibayar (QRIS)*\n\n"
            f"Order: `{order_id}`\n"
            f"User: @{_escape_md(order['username'])}\n"
            f"Jumlah: {fmt_price(order.get('total_price', order['price']))}"
        )
        await _notify_admins(bot, admin_md, exclude_user_id=order["user_id"])
        logger.info("auto_confirm: coupon service %s confirmed", order_id)
        return True
    ...
    await _notify_admins(bot, admin_md, admin_plain, exclude_user_id=order["user_id"])
