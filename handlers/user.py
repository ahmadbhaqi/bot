...
def kb_main_menu() -> InlineKeyboardMarkup:
    """Main menu: tombol utama + jasa otomasi (di luar katalog produk)."""
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🛒  Order / Beli", callback_data="menu_order")],
            [
                InlineKeyboardButton(
                    "🎟️  Jasa Klaim Kupon DO", callback_data="coupon_info"
                )
            ],
            [
...
def kb_order_menu() -> InlineKeyboardMarkup:
    ...
    Produk dengan field `hidden_in_menu: true` (mis. ghs_do & ghs_do_buyer)
    tidak ditampilkan langsung; keduanya digabung menjadi satu entri
    "Klaim DO Credit" yang akan menanyakan sumber GHS (seller/buyer) saat dipilih.
    """
    products = db.load_products()
    rows = []

    for pid, p in products.items():
        if p.get("hidden_in_menu"):
            continue
        stock = db.get_stock_count(pid)
        if db.is_service_product(pid):
            stock_lbl = "⚡ Tersedia"  # jasa: selalu tersedia
        else:
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    f"{p['emoji']}  {p['name']}  •  {fmt_price(p['price'])}  •  {stock_lbl}",
                    callback_data=f"product_{pid}",
                )
            ]
        )

    # Entri gabungan untuk jasa klaim DO (seller + buyer dalam satu menu)
    if "ghs_do" in products or "ghs_do_buyer" in products:
        prices = [
            products[pid]["price"]
            for pid in ("ghs_do", "ghs_do_buyer")
            if pid in products
        ]
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    f"⚡  DO Claim – Auto Klaim $200  •  mulai {fmt_price(start_price)}",
                    callback_data="do_merged",
                )
            ]
        )
...
def kb_product_detail_qty(
    ...
    """Keyboard halaman detail produk dengan QTY +/- untuk bulk purchase.
    Produk jasa (is_service) selalu tersedia — tombol beli tidak diblokir stok."""
    product = db.get_product(product_id)
    unit_price = product["price"] if product else 0
    is_service = db.is_service_product(product_id)
    # Untuk jasa, anggap stok "tidak terbatas" (max 10 per transaksi)
    effective_stock = 999 if is_service else stock
    ...
    back_cb = (
        "do_merged" if product_id in ("ghs_do", "ghs_do_buyer") else "menu_order"
    )
    rows.append(
...
async def _show_do_merged(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Menu gabungan jasa klaim DO: pilih sumber GHS (seller / buyer).

    Keduanya adalah jasa auto-klaim DigitalOcean $200, hanya berbeda asal akun
    GitHub Education dan harga. Masing-masing pilihan membuka detail produk
    (ghs_do = seller, ghs_do_buyer = buyer) sehingga harga tetap berbeda.
    """
    query = update.callback_query
    assert query is not None
    await query.answer()
    ...
    products = db.load_products()
    seller = products.get("ghs_do")
    buyer = products.get("ghs_do_buyer")
    ...
    if seller:
        rows.append(
            [
                InlineKeyboardButton(
                    f"🏪  GHS dari Seller  •  {fmt_price(seller['price'])}",
                    callback_data="product_ghs_do",
                )
            ]
        )
    if buyer:
        rows.append(
            [
                InlineKeyboardButton(
                    f"👤  GHS dari Buyer (GHS-mu sendiri)  •  {fmt_price(buyer['price'])}",
                    callback_data="product_ghs_do_buyer",
                )
            ]
        )
    rows.append(
    ...
    text = (
        "⚡ *DO Claim — Auto Klaim DigitalOcean $200*\n\n"
        "Bot akan otomatis mengklaim kredit DigitalOcean *$200* ke akun DO milikmu.\n\n"
        "Pilih *sumber akun GitHub Education* yang dipakai:\n\n"
        "🏪 *GHS dari Seller* — pakai akun GHS dari stok kami "
        "_(tanpa perlu punya GHS sendiri)_.\n"
        "👤 *GHS dari Buyer* — pakai akun GHS milikmu sendiri "
        "_(lebih hemat)_."
    )
...
async def _show_product_detail(
    ...
    is_service = db.is_service_product(product_id)
    is_ghs_do = product_id in ("ghs_do", "ghs_do_buyer")
    ...
    if is_ghs_do:
        source_note = (
            "_GHS dari stok seller kami._"
            if product_id == "ghs_do"
            else "_GHS dari akun GitHub Education milikmu sendiri._"
        )
        extra_info = (
    ...
    # Banner promo volume discount (per-produk)
    promo_line = ""
    promo = db.get_promo(product_id)
    if promo["min_qty"] > 0 and promo["promo_price"] > 0:
        promo_line = (
            f"\n\n🎁 *PROMO:* Beli ≥ *{promo['min_qty']}* "
            f"→ harga *{fmt_price(promo['promo_price'])}*/item!"
        )
...
async def _handle_buy(
    ...
    # Produk jasa tidak diblokir stok — selalu bisa dibeli
    if not is_service:
        if stock <= 0:
    ...
    # Hitung total dengan mempertimbangkan promo volume discount per-produk
    total_price = db.calc_promo_total(product_id, product["price"], qty)
    ...
    # Tampilkan pilihan metode pembayaran: Saldo atau Payment Gateway (QRIS).
    await _show_payment_choice(query, order_id, product, qty, total_price)


async def _show_payment_choice(
    query, order_id: str, product: dict, qty: int, total_price: int
) -> None:
    """Tampilkan pilihan metode bayar: saldo lokal atau QRIS (Pakasir)."""
    user = query.from_user
    balance = db.get_balance(user.id) if user else 0
    qty_line = f"🔢 Jumlah: *{qty}x*\n" if qty > 1 else ""
    ...
    text = (
        ...
        f"💰 Saldo kamu: *{fmt_price(balance)}*\n\n"
        f"Pilih cara pembayaran di bawah:"
    )
    ...
    if balance >= total_price:
        rows.append(
            [
                InlineKeyboardButton(
                    f"💰 Bayar pakai Saldo ({fmt_price(total_price)})",
                    callback_data=f"paysaldo_{order_id}",
                )
            ]
        )
    else:
        rows.append(
            [
                InlineKeyboardButton(
                    "💰 Saldo tidak cukup", callback_data="noop"
                )
            ]
        )
    if PAKASIR_ENABLED and _pakasir_client:
        rows.append(
            [
                InlineKeyboardButton(
                    "📲 Bayar via QRIS", callback_data=f"paygw_{order_id}"
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton("❌ Batalkan", callback_data=f"cancel_order_{order_id}")]
    )
...
async def _handle_pay_saldo(
    update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str
) -> None:
    """Bayar order memakai saldo lokal. Saldo dipotong, lalu order langsung
    dikonfirmasi & produk dikirim (memakai auto_confirm_order)."""
    query = update.callback_query
    assert query is not None
    user = update.effective_user
    assert user is not None
    ...
    amount = order.get("total_price", order["price"])
    if not db.deduct_balance(user.id, amount):
        await query.answer(
            "Saldo tidak cukup! Top up dulu atau pilih QRIS.", show_alert=True
        )
        return

    # Tandai metode bayar saldo agar tidak di-poll gateway
    db.update_order(order_id, payment_method="saldo")
    await query.answer("✅ Saldo dipotong, memproses pesanan...")

    from handlers.admin import auto_confirm_order

    ok = await auto_confirm_order(order_id, context.bot)
    if not ok:
        # Gagal (mis. stok habis) → kembalikan saldo
        db.add_balance(user.id, amount, user.username or "")
        await _edit_or_reply_text(
            query,
            "⚠️ *Pesanan gagal diproses* (stok habis atau kendala lain).\n"
            f"💰 Saldo *{fmt_price(amount)}* sudah dikembalikan.\n\n"
    ...
    try:
        await _edit_or_reply_text(
            ...
            "Pesananmu sedang/ sudah diproses. Cek pesan dari bot di chat ini.",
            parse_mode="Markdown",
            reply_markup=kb_back_main(),
        )
    except Exception:
...
async def _handle_pay_gateway(
    update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str
) -> None:
    """Lanjutkan pembayaran order via Pakasir (QRIS)."""
    query = update.callback_query
    assert query is not None
    user = update.effective_user
    assert user is not None
    ...
    if not (PAKASIR_ENABLED and _pakasir_client):
        await query.answer("Gateway pembayaran tidak tersedia.", show_alert=True)
        return
    ...
    try:
        ...
        logger.error("Pakasir gagal untuk order %s: %s", order_id, exc)
        await _edit_or_reply_text(
            query,
            "⚠️ Gagal membuat transaksi pembayaran. Coba lagi nanti.",
            reply_markup=kb_back_main(),
        )
...
async def _handle_buy_pakasir(
    ...
    if txn.is_qris and txn.payment_number:
        ...
        try:
            sent_msg = await msg.chat.send_photo(
                photo=qr_url,
                caption=short_caption,
                parse_mode="Markdown",
                reply_markup=new_kb,
            )
            # Simpan id pesan QRIS agar bisa dihapus otomatis setelah lunas
            try:
                db.update_order(
                    order_id,
                    pakasir_qris_msg_id=sent_msg.message_id,
                    pakasir_qris_chat_id=sent_msg.chat_id,
                )
            except Exception:
                pass
...
async def _handle_buy_manual(query, order_id: str, product: dict, qty: int = 1, total_price: int = 0) -> None:
    """DEPRECATED — transfer manual dihapus. Arahkan user ke QRIS / saldo."""
    await _edit_or_reply_text(
        query,
        "ℹ️ Transfer manual sudah tidak tersedia. Gunakan *QRIS* atau *Saldo*.",
        parse_mode="Markdown",
        reply_markup=kb_back_main(),
    )
...
async def _handle_coupon_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Tampilkan info jasa klaim kupon DO (di luar katalog produk)."""
    from handlers.coupon_claim import show_coupon_info

    await show_coupon_info(update, context)
...
# Nominal top-up cepat yang ditawarkan ke user (Rupiah)
_TOPUP_OPTIONS = [10000, 25000, 50000, 100000, 200000]

# Conversation state untuk nominal custom
WAITING_TOPUP_CUSTOM = 401


async def _show_topup_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Tampilkan pilihan nominal top-up saldo."""
    query = update.callback_query
    assert query is not None
    await query.answer()
    ...
    if not (PAKASIR_ENABLED and _pakasir_client):
        await _edit_or_reply_text(
            query,
            "⚠️ Top up otomatis sedang tidak tersedia. Hubungi admin.",
            reply_markup=kb_back_main(),
        )
        return

    rows = []
    for amt in _TOPUP_OPTIONS:
        rows.append(
            [
                InlineKeyboardButton(
                    f"💵  {fmt_price(amt)}", callback_data=f"topup_amt_{amt}"
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton("✏️  Nominal Lain (custom)", callback_data="topup_custom")]
    )
    rows.append([InlineKeyboardButton("🔙  Kembali ke Profil", callback_data="menu_profil")])

    text = (
        f"💰 *Top Up Saldo*\n\n"
        f"Saldo kamu sekarang: *{fmt_price(balance)}*\n\n"
        f"Pilih nominal top-up atau masukkan nominal sendiri.\n"
        f"Saldo masuk otomatis setelah pembayaran QRIS terdeteksi."
    )
    await _edit_or_reply_text(
        query, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(rows)
    )


async def entry_topup_custom(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> int:
    """Entry ConversationHandler: user klik 'Nominal Lain' → minta ketik nominal."""
    query = update.callback_query
    assert query is not None
    await query.answer()
    try:
    ...
    await update.effective_chat.send_message(
        "💰 *Top Up Saldo — Nominal Custom*\n\n"
        "Ketik nominal top-up yang kamu inginkan (minimal Rp 1.000):\n\n"
        "Contoh: `75000` atau `150000`\n\n"
        "Ketik /cancel untuk membatalkan.",
        parse_mode="Markdown",
    )
    return WAITING_TOPUP_CUSTOM


async def handle_topup_custom_input(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> int:
    """Terima nominal custom, validasi, buat order top-up."""
    msg = update.message
    raw = (msg.text or "").strip().replace(".", "").replace(",", "")

    if not raw.isdigit():
        await msg.reply_text(
            "❌ Nominal tidak valid. Masukkan angka saja (tanpa titik/koma).\n\n"
    ...
    if amount < 1000:
        await msg.reply_text(
            f"❌ Nominal minimal Rp 1.000. Kamu memasukkan *{fmt_price(amount)}*.\n\n"
            "Coba lagi, atau /cancel.",
            parse_mode="Markdown",
        )
        return WAITING_TOPUP_CUSTOM
    ...
    if amount > 10_000_000:
        await msg.reply_text(
            "❌ Nominal terlalu besar (maks Rp 10.000.000).\n\nCoba lagi, atau /cancel.",
        )
        return WAITING_TOPUP_CUSTOM
    ...
    if not (PAKASIR_ENABLED and _pakasir_client):
        await msg.reply_text(
            "⚠️ Gateway pembayaran tidak tersedia saat ini. Hubungi admin.",
            reply_markup=kb_back_main(),
        )
        return ConversationHandler.END
    ...
    order_id = db.create_topup_order(user.id, user.username or user.first_name, amount)
    pseudo_product = {"name": "Top Up Saldo", "price": amount}

    # Buat objek 'shim' — handle_buy_pakasir butuh query-like object.
    # Karena ini berasal dari message (bukan callback), kita kirim pesan baru
    # sebagai titik awal lalu oper ke helper.
    placeholder = await msg.reply_text("⏳ Membuat QRIS pembayaran...")

    class _MsgShim:
        """Shim agar _handle_buy_pakasir bisa menghapus & mengirim ulang."""
        def __init__(self):
        ...
        async def edit_message_text(self, *a, **kw):
            raise Exception("shim: no edit")

    shim = _MsgShim()
    try:
        await _handle_buy_pakasir(shim, order_id, pseudo_product, 1, amount)
    except Exception as exc:
        logger.error("Top-up custom QRIS gagal order %s: %s", order_id, exc)
        try:
            await placeholder.edit_text(
                "⚠️ Gagal membuat transaksi top-up. Coba lagi nanti.",
                reply_markup=kb_back_main(),
            )
        except Exception:
            await msg.reply_text(
...
async def _handle_topup_amount(
    update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str
) -> None:
    """Buat order top-up dengan nominal terpilih, lalu tampilkan QRIS Pakasir."""
    query = update.callback_query
    assert query is not None
    user = update.effective_user
    assert user is not None
    ...
    # Bungkus sebagai "produk" semu untuk dipakai _handle_buy_pakasir
    pseudo_product = {"name": "Top Up Saldo", "price": amount}
    await query.answer()
    try:
        await _handle_buy_pakasir(query, order_id, pseudo_product, 1, amount)
    except Exception as exc:
        logger.error("Pakasir top-up gagal order %s: %s", order_id, exc)
        await _edit_or_reply_text(
            query,
            "⚠️ Gagal membuat transaksi top-up. Coba lagi nanti.",
            reply_markup=kb_back_main(),
...
EXACT_ROUTES = {
    ...
    "do_merged": _show_do_merged,
    "coupon_info": _handle_coupon_info,
    "menu_topup": _show_topup_menu,
...
PREFIX_ROUTES = {
    ...
    "buy_": _handle_buy,
    "paysaldo_": _handle_pay_saldo,
    "paygw_": _handle_pay_gateway,
    "topup_amt_": _handle_topup_amount,
