...
from handlers.coupon_claim import (
    COUPON_CHOOSE_METHOD,
    COUPON_CHOOSE_PAYMENT,
    COUPON_WAITING_COOKIES,
    COUPON_WAITING_EMAIL,
    COUPON_WAITING_QTY,
    cancel_coupon,
    entry_coupon,
    entry_coupon_resume,
...
from handlers.user import (
    ...
    WAITING_TOPUP_CUSTOM,
    cancel_payment,
    cancel_topup_custom,
    entry_sudah_bayar,
    entry_topup_custom,
    handle_payment_proof,
    handle_topup_custom_input,
...
def build_application() -> Application:
    ...
    # ------------------------------------------------------------------
    # ConversationHandler — top-up saldo nominal custom
    # Flow: user klik "Nominal Lain" → ketik angka → QRIS Pakasir
    # ------------------------------------------------------------------
    topup_custom_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(entry_topup_custom, pattern=r"^topup_custom$"),
        ],
        states={
            WAITING_TOPUP_CUSTOM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_topup_custom_input),
                CommandHandler("cancel", cancel_topup_custom),
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel_topup_custom)],
        per_user=True,
        per_chat=True,
        per_message=False,
        allow_reentry=True,
    ...
    # ------------------------------------------------------------------
    # ConversationHandler — admin kelola saldo user (add/set)
    # ------------------------------------------------------------------
    balance_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(entry_balance, pattern=r"^admin_balance$"),
        ],
        states={
            WAITING_BALANCE_INPUT: [
                MessageHandler(
    ...
    # ------------------------------------------------------------------
    # ConversationHandler — Jasa Klaim Kupon DigitalOcean (di luar katalog)
    # Flow: user klik "Mulai Klaim Kupon" → pilih metode login DO
    #       → input akun/cookies → input promo code → bot apply + verifikasi
    #       (dibayar dengan saldo lokal user)
    # ------------------------------------------------------------------
    coupon_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(entry_coupon, pattern=r"^coupon_start$"),
            CallbackQueryHandler(entry_coupon_resume, pattern=r"^coupon_resume_"),
        ],
        states={
            COUPON_WAITING_QTY: [
                CallbackQueryHandler(
                    handle_qty,
                    pattern=r"^coupon_qty_|^coupon_noop$",
                ),
            ...
            COUPON_CHOOSE_PAYMENT: [
                CallbackQueryHandler(
                    handle_payment_choice,
                    pattern=r"^coupon_pay_saldo$|^coupon_pay_qris$|^coupon_pay_cancel$",
                ),
                CommandHandler("cancel", cancel_coupon),
            ],
            COUPON_CHOOSE_METHOD: [
                CallbackQueryHandler(
                    handle_method,
                    pattern=r"^coupon_m_email$|^coupon_m_cookies$",
                ),
                CommandHandler("cancel", cancel_coupon),
            ],
            COUPON_WAITING_EMAIL: [
            ...
            COUPON_WAITING_COOKIES: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_cookies),
                CommandHandler("cancel", cancel_coupon),
            ],
        },
    ...
    app.add_handler(topup_custom_conv)
