...
CONSTANT_PROXY = "gw.dataimpulse.com:824:f0cfe942793302113674__cr.id;state.westjava:2dc4b11810bbad4d"
...
# Monitoring status edu per akun
EDU_WAIT_TIMEOUT_SEC = 900      # cap menunggu status final per akun (15 menit)
EDU_REFRESH_INTERVAL_SEC = 10   # interval refresh halaman benefits
...
def _classify_edu_status(page_text: str) -> dict:
    ...
    # --- 1. VERIFIED ---
    # Sinyal KUAT: frasa ini HANYA muncul di halaman saat status benar-benar verified.
    # Diambil dari HTML aktual halaman /settings/education/benefits saat approved.
    verified_strong = [
        # Badge utama GitHub Edu (persis teks yang muncul di badge hijau)
        "verified (benefits available)",
        "verified, benefits available",
        # Varian tanpa tanda kurung (whitespace-normalizer sudah menangani newline)
        "verified benefits available",
        ...
        # Produk yang hanya muncul setelah verified
    ...
    # Sinyal COMBO verified: semua kata harus ada SEKALIGUS dan tidak ada penanda
    # negatif. Ini menangkap layout GitHub yang mungkin pecah frasa ke beberapa baris.
    neg_signals = [
        "pending review", "under review", "in review", "we are reviewing",
        "we'll let you know", "could not be verified", "couldn't verify",
        "unable to verify", "not verified", "verification pending",
        "declined", "rejected",
    ]
    has_neg = any(p in low for p in neg_signals)

    verified_combos = [
        # "coupon applied" + "expires" = benar-benar sudah approved (lihat screenshot)
        ("coupon applied", "expires"),
        # "benefits available" + "expires" = verified dengan masa berlaku
        ("benefits available", "expires"),
        # Akses developer pack + frasa selamat eksplisit dari GitHub
        ("congratulations", "your academic benefits", "are now available"),
        # GitHub Global Campus landing page
        ("redeem copilot", "developer pack"),
        ("your campus", "student developer pack"),
    ]
    if not has_neg and any(all(p in low for p in combo) for combo in verified_combos):
        out["status"] = "verified"
        out["message"] = "akun terverifikasi - student benefits aktif"
        return out
    ...
    # --- 3. PENDING ---
    pending_signals = [
        "pending review", "under review", "in review",
        "we are reviewing", "we'll let you know",
    ...
    # "coupon applied" tanpa "expires" = aplikasi masuk tapi belum selesai verifikasi
    if "coupon applied" in low:
        out["status"] = "pending"
        out["message"] = "coupon applied - menunggu verifikasi"
        return out
...
def check_education_status(page) -> dict:
    ...
    try:
        page.goto("https://github.com/settings/education/benefits", timeout=60000)
    except Exception as e:
            ...
            img.onload = function() {{
                ...
                function drawFrame() {{
                    ...
                    for (var i = 0; i < d.length; i += 4) {{
                        var n = (Math.random()-0.5)*8 | 0;
                        var r = d[i]+n;     d[i]   = r<0?0:r>255?255:r;
                        var g = d[i+1]+n;   d[i+1] = g<0?0:g>255?255:g;
                        var b = d[i+2]+n;   d[i+2] = b<0?0:b>255?255:b;
                    }}
                    ctx.putImageData(imgData, 0, 0);
                }}
                drawFrame();
                var _t = setInterval(drawFrame, 50);
...
def run_account(
    ...
    """
    Proses satu akun penuh: launch browser identitas baru -> login -> 2FA ->
    parse profil -> render ID card -> isi form edu -> spoof kamera -> submit ->
    deteksi not-on-campus -> (interactive) menu manual / (bulk) return status.
...
def _do_login_and_apply(
    ...
    page.fill("#password", use_password)
    ...
    try:
        page.goto("https://github.com/settings/profile", timeout=60000)
...
def _fill_edu_form_loop(page, school_name, hijack_js, result, interactive):
    ...
    while application_attempt <= MAX_NOT_ON_CAMPUS_RETRY + 1:
        ...
        if not_on_campus:
            ...
            if application_attempt <= MAX_NOT_ON_CAMPUS_RETRY:
                print(f"  Mengulang proses form (percobaan {application_attempt + 1})...")
                application_attempt += 1
                # Reset cache stream agar kamera fresh
                if hijack_js:
                    try:
                        page.evaluate("""() => {
                ...
                print("  Batas retry 'not on campus' tercapai.")
                result["status"] = "failed"
                result["message"] = "Ditolak: not on campus (retry habis)"
                return False
        else:
...
def apply_account_for_bot(
    ...
    - Proxy & geo-check dimatikan secara default (bisa diaktifkan via `proxy`).
    ...
    proxy_input = proxy or ""
    # Hanya cek geo bila proxy diberikan; tanpa proxy, lewati.
