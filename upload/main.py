...
def build_application() -> Application:
    ...
    # ------------------------------------------------------------------
    # ConversationHandler — admin tambah promo (beli N gratis 1)
    # Flow: admin klik "Atur Promo" → kirim jumlah akun pembelian
    # ------------------------------------------------------------------
    promo_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(entry_promo, pattern=r"^admin_promo_set$"),
        ],
        states={
            WAITING_PROMO_INPUT: [
                MessageHandler(
    ...
    do_claim_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(
                entry_do_claim,
                pattern=r"^do_claim_email_|^do_claim_cookies_",
            ),
        ],
        states={
            DO_WAITING_EMAIL: [
